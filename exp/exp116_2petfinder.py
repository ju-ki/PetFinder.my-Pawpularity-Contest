...
def train_fn(fold, train_loader, model, criterion, optimizer, epoch, scheduler, device):
    ...
    for step, (images, labels) in enumerate(train_loader):
        ...
        if mixup_decision < 0.5:
            if Config.resizemix:
                images, target_a, target_b, lam = resizemix(images, labels, alpha=0.4, beta=0.5)
    ...
    if Config.grad_cam:
        for fold in range(Config.n_folds):
            if fold in Config.trn_fold:
                ...
                model.eval()
                # load oof
                oof = pd.read_csv(OUTPUT+'/oof_df1.csv')
...
# oof_df = pd.read_csv("./models/exp116/")
# oof1_df = pd.read_csv("./models/exp116/oof_df.csv")
# oof_df = pd.concat([oof1_df, oof_df])
# score = get_score(oof_df["Pawpularity"].values, oof_df["preds"].values)
# logger.info(f"Total_score:{score:.5f}")
# oof_df.to_csv(OUTPUT + "/oof_df.csv", index=False)
