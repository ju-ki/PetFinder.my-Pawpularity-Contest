...
def get_optimizer(model: nn.Module, Config: dict):
    if Config.optimizer_name == "AdamW":
        ...
        optimizer = SGD(model.parameters(), lr=Config.lr,
                        weight_decay=Config.weight_decay)
    elif Config.optimizer_name == "SAM":
            ...
            if Config.optimizer_name == "SAM":
                if mixup_decision < 0.25:
                    if Config.mixup:
                        ...
                        y_preds = model(images.float())
                        mixup_criterion(criterion, y_preds.view(-1), target_a, target_b, lam).backward()
                    ...
                    if Config.cutmix:
                        ...
                        optimizer.first_step(zero_grad=True)
                        y_preds = model(images.float())
                        cutmix_criterion(criterion, y_preds.view(-1), target_a, target_b, lam).backward()
                        optimizer.second_step(zero_grad=True)
                    elif Config.fmix:
                        loss = mixup_criterion(criterion, y_preds.view(-1), target_a, target_b, lam)
                        loss.backward()
                    ...
                    optimizer.first_step(zero_grad=True)
                    y_preds = model(images.float())
                    criterion(y_preds.view(-1), labels).backward()
                    optimizer.second_step(zero_grad=True)
        # record loss
