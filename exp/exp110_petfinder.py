...
class CustomModel(nn.Module):
    ...
    def feature(self, image):
        if self.cfg.model_name == "resnet50d":
    ...
    def forward(self, image):
        if self.cfg.model_name == "resnet50d":
            ...
            x = self.pool(features).view(image.size(0), -1)
            x = self.dropout(x)
            x = self.classifier(x)
            return x, features
        else:
...
def train_fn(fold, train_loader, model, criterion, optimizer, epoch, scheduler, device):
    ...
    for step, (images, labels) in enumerate(train_loader):
        ...
        images, target_a, target_b, lam_a, lam_b = snapmix(images, labels, Config, alpha=1.0,  model=model)
        ...
        if Config.apex:
            ...
            if Config.model_name == "resnet50d":
...
def snapmix(input,target,conf, alpha, model=None):
