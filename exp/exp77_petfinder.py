...
def make_simsiam_model():
    resnet = timm.create_model(Config.model_name, num_classes=512, pretrained=True)
    model = SimSiam(resnet, out_dim=512, pred_dim=256)
    model.load_state_dict(torch.load("./models/simsi/new_simsiam_resnet34d_160_model.pth", map_location=torch.device("cpu")))
    ...
    def __init__(self, cfg, pretrained=False):
        ...
        if Config.simsiam:
            ...
            if cfg.model_name == "tf_efficientnet_b0_ns":
                ...
                self.fc = nn.Sequential(nn.Dropout(0.2), nn.Linear(self.n_features, self.cfg.target_size))
            elif cfg.model_name == "seresnet50":
                self.n_features = self.model.get_classifier().in_features
                self.model.reset_classifier(0)
                self.fc = nn.Sequential(nn.Dropout(0.2), nn.Linear(self.n_features, self.cfg.target_size))
            elif cfg.model_name == "resnet34d":
                self.n_features = self.model.get_classifier().in_features
                self.model.reset_classifier(0)
                self.fc = nn.Sequential(nn.Dropout(0.2), nn.Linear(self.n_features, self.cfg.target_size))
            else:
...
class SimSiam(nn.Module):
    def __init__(self, base_encoder, out_dim=2048, pred_dim=512):
        ...
        prev_dim = self.encoder.fc.weight.shape[1]#resnet50d ->2048, resnet34d, resnet18 -> 512
        "3-layer predictor"
...
def train_fn(fold, train_loader, model, criterion, optimizer, epoch, scheduler, device):
    ...
    for step, (images, labels) in enumerate(train_loader):
        ...
        if mixup_decision < 0.25:
            if Config.mixup:
                images, target_a, target_b, lam = mixup(images, labels)
