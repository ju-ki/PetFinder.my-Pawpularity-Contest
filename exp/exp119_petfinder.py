...
def set_environment() -> None:
    ...
    if IN_COLAB:
        ...
        import condacolab
        condacolab.install()
        get_ipython().system(u'python rapidsai-csp-utils/colab/install_rapids.py stable')
        import os
...
def get_train_file_path(image_id):
    if IN_COLAB:
        ...
        return "../input/petfinder-pawpularity-score-clean/train/{}.jpg".format(image_id)
...
class CustomModel(nn.Module):
    def __init__(self, cfg, pretrained=False):
        ...
        if cfg.model_name == "tf_efficientnet_b0_ns":
            ...
            self.model.reset_classifier(0)
            self.fc = nn.Sequential(nn.Dropout(0.5), nn.Linear(self.n_features, self.cfg.target_size))
...
class EarlyStopping:
    ...
    def __call__(self, loss, model=None, preds=None, path=None):
    ...
    def save_checkpoint(self, model=None,  preds=None, path=None):
        if path:
...
def train_loop(train_df, fold):
    ...
    for epoch in range(Config.epochs):
        ...
        clf = SVR(C=20.0)
        clf.fit(preds, valid_labels)
        svr_preds = clf.predict(preds).reshape(preds.shape[0], -1)
        ensemble_preds = (preds + svr_preds) / 2
        

        # scoring
        score = get_score(valid_labels, preds)
        svr_score = get_score(valid_labels, svr_preds)
        ensemble_score = get_score(valid_labels, ensemble_preds)
        ...
        logger.info(f'Epoch {epoch+1} - NN_Score: {score:.4f}   SVR_Score: {svr_score:.4f}   Ensemble_Score: {ensemble_score:.4f}')
        if ensemble_score < best_score:
            best_score = ensemble_score
            logger.info(f'Epoch {epoch+1} - Save Best Score: {best_score:.4f} Model')
            pickle.dump(clf, open(OUTPUT + f"/SVR_fold_{fold}_best.pkl", "wb"))
            np.save(OUTPUT + f"/SVR_fold_{fold}_best.npy", ensemble_preds)
            # torch.save({'model': model.state_dict(), 
            #             'preds': preds},
            #             OUTPUT+f'/{Config.model_name}_fold{fold}_best.pth')

        early_stopping(ensemble_score)
    ...
    clf = pickle.load(open(OUTPUT + f"/SVR_fold_{fold}_best.pkl", "rb"))
    preds = np.load(OUTPUT + f"/SVR_fold_{fold}_best.npy")
    valid_folds["preds"] = preds
    valid_folds["Pawpularity"] = valid_folds["Pawpularity"] * 100
    del preds, ensemble_preds, svr_preds
    gc.collect()
    return valid_folds
