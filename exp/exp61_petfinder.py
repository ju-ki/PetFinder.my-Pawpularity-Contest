...
class CustomModel(nn.Module):
    def __init__(self, cfg, pretrained=False):
        ...
        if cfg.model_name == "tf_efficientnet_b0_ns":
            ...
            self.backbone = nn.Sequential(*self.model.children())[:-2]
    ...
    def forward(self, image):
        ...
        feature = self.pool(feat).view(feat.size(0), -1)
...
def get_spm(input,target,model):
    ...
    with torch.no_grad():
        ...
        clslogit = clslogit.to(torch.long)
        logitlist = []
...
def snapmix_criterion(criterion, pred, target_a, target_b, lam_a, lam_b):
    ...
    loss = snapmix_loss(criterion, pred, target_a, taret_b, lam_a, lam_b)
    return loss
...
def train_fn(fold, train_loader, model, criterion, optimizer, epoch, scheduler, device):
    ...
    for step, (images, labels) in enumerate(train_loader):
        ...
        if Config.mixup:
            ...
            target_a, target_b = target_a.to(device), target_b.to(device)
        elif Config.snap_mix:
            images, target_a, target_b, lam_a, lam_b = snapmix(images, labels, alpha=1.0, model=model)
            target_a, target_b = target_a.to(device), target_b.to(device)
        ...
        if Config.apex:
            ...
            if Config.mixup:
                ...
                loss = snapmix_criterion(criterion, y_preds.view(-1), target_a, target_b, lam_a, lam_b)
            else:
                loss = criterion(y_preds.view(-1), labels)
        # record loss
...
# nn.Sequential(*tmp_model.children())[:-2]


# In[ ]:


tmp_model.fc.in_features


# In[ ]:


tmp_model.get_classifier().in_features
