...
if IN_COLAB:
    ...
    test_df = pd.read_csv("./data/external/clean_petfinder_dataset/test.csv")
elif IN_KAGGLE:
...
class TrainDataset(Dataset):
    def __init__(self, df, transform=None):
        ...
        self.dense_feature = df[Config.dense_feature].values
    ...
    def __getitem__(self, idx):
        ...
        feature = torch.tensor(self.dense_feature[idx, :]).float()
...
class TestDataset(Dataset):
    ...
    def __getitem__(self, idx):
        ...
        if self.transform:
            image = self.transform(image=image)['image']
        feature = self.dense_feature[idx, :].float()
...
class CustomModel(nn.Module):
    def __init__(self, cfg, pretrained=False):
        ...
        if cfg.model_name == "tf_efficientnet_b0_ns":
            ...
            self.head = nn.Sequential(
                nn.Linear(140, 64),
                nn.ReLU(),
                nn.Linear(64, self.cfg.target_size)
            )
    ...
    def forward(self, image, dense_feature):
        feature = self.feature(image)
        feature = self.fc(feature)
        x = self.dropout(feature)
        x = torch.cat([x, dense_feature], dim=1)
        output = self.head(x)
...
def train_fn(fold, train_loader, model, criterion, optimizer, epoch, scheduler, device):
    ...
    start = end = time.time()
    global_step = 0
    for step, (images, labels, feature) in enumerate(train_loader):
...
def valid_fn(valid_loader, model, criterion, device):
    ...
    all_labels = []
    start = end = time.time()
    for step, (images, labels, feature) in enumerate(valid_loader):
        images = images.to(device)
        labels = labels.to(device)
        feature = feature.to(device)
        batch_size = labels.size(0)
